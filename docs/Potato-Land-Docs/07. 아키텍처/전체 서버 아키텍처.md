...
```text
auction:state:{listingIdx} -> currentPrice, highestBidderIdx, highestBidIdx, minNextBid, updatedAt
```

경매 상태, 시작가, 종료 시간, 입찰자 목록은 PostgreSQL을 기준으로 조회한다.
...
sequenceDiagram
    ...
    API->>N: LISTING_DELETED/AUCTION_LEADER_CHANGED 알림 저장 및 push
```
