...
> canonical 업무 API는 **68개**다. `/health`와 인증 session 관리 보조 URL은 별도다.
...
| 45 | 후기 | `GET` | `/api/reviews/tags` | 공개 | `listReviewTags → review_tags active order` |
| 46 | 후기 | `POST` | `/api/reviews` | 필수 | `createReview → transaction lock → tags/notification → Socket` |
| 47 | 후기 | `GET` | `/api/users/:userIdx/reviews` | 공개 | `listUserReviews → listReceivedReviews → tags aggregate` |
| 48 | 마이페이지 | `GET` | `/api/mypage/me/listings` | 필수 | `listMyListings → findListingsBySeller` |
| 49 | 마이페이지 | `GET` | `/api/mypage/me/favorites` | 필수 | `listMyFavorites → findFavoritesByUser` |
| 50 | 마이페이지 | `GET` | `/api/mypage/me/history` | 필수 | `listMyHistory → transaction/active bid UNION` |
| 51 | 마이페이지 | `GET` | `/api/mypage/me/reviews` | 필수 | `listMyReviews → written/received filter` |
| 52 | 마이페이지 | `GET` | `/api/mypage/:userIdx/listings` | 공개 | `listUserListings → findListingsBySeller` |
| 53 | 알림 | `GET` | `/api/notifications` | 필수 | `notifications list → pagination` |
| 54 | 알림 | `PATCH` | `/api/notifications/:notificationIdx/read` | 필수 | `owner notification → read_at/is_read → unread Socket` |
| 55 | 알림 | `PATCH` | `/api/notifications/read-all` | 필수 | `receiver unread rows 일괄 update → Socket` |
| 56 | 알림 | `GET` | `/api/notifications/unread-count` | 필수 | `receiver unread COUNT` |
| 57 | 관리자 | `GET` | `/api/admin/dashboard` | ADMIN | `findDashboard → summary/time series SQL` |
| 58 | 관리자 | `GET` | `/api/admin/users` | ADMIN | `listUsers → search/status/page` |
| 59 | 관리자 | `GET` | `/api/admin/users/:userIdx` | ADMIN | `getUser → account/summary SQL` |
| 60 | 관리자 | `GET` | `/api/admin/users/:userIdx/transactions` | ADMIN | `listTransactions → user activity SQL` |
| 61 | 관리자 | `GET` | `/api/admin/users/:userIdx/reviews` | ADMIN | `listReviews → written/received activity SQL` |
| 62 | 관리자 | `PATCH` | `/api/admin/users/:userIdx/ban` | ADMIN | `banUser → DB cascade → session/Timer/Redis` |
| 63 | 관리자 | `PATCH` | `/api/admin/users/:userIdx/memo` | ADMIN | `updateUserMemo → users.admin_memo` |
| 64 | 관리자 | `GET` | `/api/admin/used` | ADMIN | `listAdminListings(USED)` |
| 65 | 관리자 | `DELETE` | `/api/admin/used/:listingIdx` | ADMIN | `used.service.deleteUsedListing 재사용` |
| 66 | 관리자 | `GET` | `/api/admin/auctions` | ADMIN | `listAdminListings(AUCTION)` |
| 67 | 관리자 | `DELETE` | `/api/admin/auctions/:listingIdx` | ADMIN | `auctions.service.deleteAuction 재사용` |
| 68 | 관리자 | `GET` | `/api/admin/auctions/winners` | ADMIN | `findAuctionWinners` |

## 호환 및 보조 URL

- `PATCH /api/users/me/password`: `PATCH /api/users/me`의 deprecated 입력 호환 alias다. canonical 68개에는 포함하지 않는다.
- 인증 session 관리 URL(`logout-all`, `sessions`)은 운영 보조 API이며 canonical 업무 68개와 별도로 유지한다.
