...
| `AUCTION_LEADER_CHANGED` | 경매 상세 | 최고 입찰자가 비활성화되어 최고 입찰자 재계산 |
| `LISTING_DELETED` | 알림 또는 마이페이지 | 관련 게시글이 삭제됨 |
| `PAYMENT_REQUESTED` | 거래내역/채팅 | 판매자가 송금 요청 생성 |
| `PAYMENT_RECEIVED` | 거래내역 | 구매자가 송금 완료하여 판매자가 수신 |
| `PAYMENT_CANCELED` | 거래내역/채팅 | 판매자가 송금 요청 취소 |
