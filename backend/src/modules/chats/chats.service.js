  ...
  const { rows } = await executor.query(
    `
      SELECT
        ...
        cr.listing_idx AS "listingIdx",
        cr.buyer_idx AS "buyerIdx",
        cr.created_at AS "createdAt",
        cr.updated_at AS "updatedAt",
        cr.last_message_at AS "lastMessageAt",
        l.seller_idx AS "sellerIdx",
        l.listing_type AS "listingType",
        l.title AS "listingTitle",
        l.deleted_at AS "listingDeletedAt",
        seller.idx AS "sellerUserIdx",
        seller.nickname AS "sellerNickname",
        seller.profile_image AS "sellerProfileImageUrl",
        seller.deleted_at AS "sellerDeletedAt",
        seller.banned_at AS "sellerBannedAt",
        buyer.idx AS "buyerUserIdx",
        buyer.nickname AS "buyerNickname",
        buyer.profile_image AS "buyerProfileImageUrl",
        buyer.deleted_at AS "buyerDeletedAt",
        buyer.banned_at AS "buyerBannedAt",
        used_post.price AS "usedPrice",
        used_post.trade_status AS "usedTradeStatus",
        auction_post.current_price AS "auctionCurrentPrice",
        auction_post.status AS "auctionStatus",
        auction_post.ends_at AS "auctionEndsAt",
        active_transaction.idx AS "transactionIdx",
        active_transaction.status AS "transactionStatus",
...
/** 요청 사용자가 채팅방의 판매자 또는 구매자인지 확인한다. */
export async function getChatRoomForParticipant(
  chatRoomIdx,
  userIdx,
  executor = query,
) {
  ...
  if (!room) {
    throw notFound("채팅방을 찾을 수 없습니다.");
  }

  const isSeller = toId(room.sellerIdx) === Number(userIdx);
  const isBuyer = toId(room.buyerIdx) === Number(userIdx);
...
/** 게시글·거래·사용자 상태를 기준으로 채팅 쓰기 가능 여부를 계산한다. */
export function getChatWriteState(room) {
  const seller = {
    deletedAt: room.sellerDeletedAt,
    bannedAt: room.sellerBannedAt,
  };
  const buyer = {
    deletedAt: room.buyerDeletedAt,
    bannedAt: room.buyerBannedAt,
  };

  if (!isUserActive(seller) || !isUserActive(buyer)) {
    return {
      canSendMessage: false,
      readOnlyReason: "INACTIVE_USER",
  ...
  if (room.listingDeletedAt) {
    return {
      canSendMessage: false,
      readOnlyReason: "LISTING_DELETED",
      message: "삭제된 게시글의 채팅방입니다.",
    };
  }

  if (
    room.listingType === "USED"
    && room.usedTradeStatus === "ON_SALE"
  ) {
    return {
      canSendMessage: true,
      readOnlyReason: null,
...
export function assertChatWritable(room) {
  ...
  if (writeState.canSendMessage) {
    return;
  }

  throw conflict(
    writeState.readOnlyReason === "INACTIVE_USER"
      ? "INACTIVE_USER"
      : "READ_ONLY_CHAT",
    writeState.message,
  );
...
/** 채팅방에 연결된 읽지 않은 새 메시지 알림 수를 조회한다. */
  ...
  const { rows } = await executor.query(
    `
      ...
      JOIN chat_messages message
        ON message.idx = notification.reference_idx
...
/** 게시글 판매자와 구매자의 채팅방을 생성하거나 기존 방을 반환한다. */
export async function createChatRoom(listingIdx, buyerIdx) {
  ...
  return withTransaction(async (client) => {
    ...
    if (!listing || listing.listingDeletedAt) {
      throw notFound("게시글을 찾을 수 없습니다.");
    }

    if (toId(listing.sellerIdx) === Number(buyerIdx)) {
      throw forbidden("본인 게시글에는 채팅을 만들 수 없습니다.");
    }

    if (listing.sellerDeletedAt || listing.sellerBannedAt) {
      throw conflict(
        "INACTIVE_USER",
        "비활성화된 사용자와 새 채팅방을 만들 수 없습니다.",
      );
    }

    if (
      listing.listingType === "USED"
      && listing.usedTradeStatus !== "ON_SALE"
    ) {
      throw conflict(
        "CONFLICT",
        "거래가 종료된 게시글에는 채팅을 만들 수 없습니다.",
      );
    }
    ...
    const chatRoom = inserted.rows[0]
      ?? (
        await client.query(
          `
            SELECT
    ...
    return {
      ...
      roomNewPayload: {
        ...
        listingThumbnailUrl: listing.listingThumbnailUrl,
        opponentIdx: Number(buyerIdx),
        opponentNickname: listing.buyerNickname,
        lastMessage: null,
        unreadCount: 0,
...
function messageDto(message, room) {
  const senderIsSeller =
    message.senderIdx !== null
    && Number(message.senderIdx) === toId(room.sellerIdx);
  const senderNickname = senderIsSeller
    ? room.sellerNickname
    : room.buyerNickname;
  const senderProfileImageUrl = senderIsSeller
    ? room.sellerProfileImageUrl
    : room.buyerProfileImageUrl;

  return {
    messageIdx: toId(message.messageIdx),
    chatRoomIdx: toId(message.chatRoomIdx),
    senderIdx:
      message.senderIdx === null ? null : toId(message.senderIdx),
    senderNickname:
      message.senderIdx === null ? null : senderNickname,
    senderProfileImageUrl:
      message.senderIdx === null ? null : senderProfileImageUrl,
    messageType: message.messageType,
    content: message.content,
    clientMessageId: message.clientMessageId,
    imageUrl: message.imageUrl,
    transactionIdx:
      message.transactionIdx === null
        ? null
        : toId(message.transactionIdx),
    createdAt: message.createdAt,
  };
}

function getReceiverIdx(room, senderIdx) {
  return toId(room.sellerIdx) === Number(senderIdx)
    ? toId(room.buyerIdx)
    : toId(room.sellerIdx);
}
...
export function isRecipientViewingChat(io, receiverIdx, chatRoomIdx) {
  ...
  for (const socket of io.sockets.sockets.values()) {
    const isReceiver =
      Number(socket.data.user?.userIdx) === Number(receiverIdx);
    const isViewingRoom =
      Number(socket.data.activeChatRoomIdx) === Number(chatRoomIdx);

    if (isReceiver && isViewingRoom) {
      return true;
    }
  }
...
/** 텍스트 메시지를 멱등하게 저장하고 필요할 때 알림을 생성한다. */
export async function createTextMessage({ io, userIdx, payload }) {
  const chatRoomIdx = parsePositiveInteger(
    payload?.chatRoomIdx,
    "chatRoomIdx",
  );
  ...
  if (
    ...
    throw new AppError({
      status: 400,
      code: "VALIDATION_ERROR",
      message: "메시지 입력값을 확인해주세요.",
    });
  }

  return withTransaction(async (client) => {
    const room = await getChatRoomForParticipant(
      chatRoomIdx,
      userIdx,
      client,
    );
    ...
    const existing = await client.query(
      `
        SELECT
          ...
          transaction_idx AS "transactionIdx",
          content,
    ...
    if (existing.rowCount === 1) {
      return {
        created: false,
        room,
        message: messageDto(existing.rows[0], room),
        notification: null,
      };
    }
...
/** 업로드된 이미지마다 채팅 메시지를 생성하고 마지막 이미지에 알림을 연결한다. */
export async function createImageMessages({
  io,
  userIdx,
  chatRoomIdx,
  files,
  ...
  return withTransaction(async (client) => {
    ...
    await client.query(
      `
        ...
        WHERE idx = $1
      `,
      [roomId, messages.at(-1).createdAt],
    );
...
export function toChatDetail(room, currentUserIdx, unreadCount) {
  const writeState = getChatWriteState(room);
  const sellerActive = isUserActive({
    deletedAt: room.sellerDeletedAt,
    bannedAt: room.sellerBannedAt,
  });
  const buyerActive = isUserActive({
    deletedAt: room.buyerDeletedAt,
    bannedAt: room.buyerBannedAt,
  });
  ...
  const person = (
    ...
    userIdx: toId(userIdx),
    nickname: active ? nickname : "삭제된 사용자",
    displayName: active ? nickname : "삭제된 사용자",
    profileImageUrl: active ? profileImageUrl : null,
    profileEnabled: active,
  });

  const canCreatePaymentRequest =
    Number(currentUserIdx) === toId(room.sellerIdx)
    && !room.transactionIdx
    && !room.listingDeletedAt
    && sellerActive
    && buyerActive
    && room.listingType === "USED"
    && room.usedTradeStatus === "ON_SALE";
  const displayPrice = room.listingType === "USED"
    ? room.usedPrice
    : room.auctionCurrentPrice;

  return {
    chatRoomIdx: toId(room.chatRoomIdx),
    listing: {
      listingIdx: toId(room.listingIdx),
      listingType: room.listingType,
      title: room.listingTitle,
      thumbnailUrl: room.listingThumbnailUrl,
      displayPrice:
        displayPrice === null ? null : Number(displayPrice),
      status:
        room.listingType === "USED"
          ? room.usedTradeStatus
          : room.auctionStatus,
      deleted: Boolean(room.listingDeletedAt),
      detailLinkEnabled: !room.listingDeletedAt,
    },
    seller: person(
      room.sellerUserIdx,
      room.sellerNickname,
    ...
    unreadCount,
    canSendMessage: writeState.canSendMessage,
    canCreatePaymentRequest,
    readOnly: !writeState.canSendMessage,
    readOnlyReason: writeState.readOnlyReason,
  };
}

/** 저장된 메시지 row를 공통 응답 DTO로 변환한다. */
export function toChatMessageDto(row) {
  return {
    messageIdx: toId(row.messageIdx),
    chatRoomIdx: toId(row.chatRoomIdx),
    senderIdx: row.senderIdx === null ? null : toId(row.senderIdx),
    messageType: row.messageType,
    content: row.content,
    clientMessageId: row.clientMessageId,
    imageUrl: row.imageUrl,
    transactionIdx:
      row.transactionIdx === null ? null : toId(row.transactionIdx),
