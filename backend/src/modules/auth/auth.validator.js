...
function validatePhoneBase(body = {}) {
  ...
  if (!PHONE_PATTERN.test(phone)) throw new AppError({
    status: 400,
    code: "VALIDATION_ERROR",
    message: "휴대전화 번호 형식을 확인해주세요.",
    details: { field: "phone" }
  });
  if (!PHONE_PURPOSES.has(purpose)) throw new AppError({
    status: 400,
    code: "VALIDATION_ERROR",
    message: "올바른 휴대전화 인증 목적이 아닙니다.",
...
export function validateSignup(body = {}) {
  ...
  const requiredFields = [
    "name", "nickname", "loginId", "password", "passwordConfirm", "phone", "phoneVerificationId"
  ];

  const missingField = requiredFields.find((field) => !data[field]);

  if (missingField) throw new AppError({
    status: 400,
    code: "VALIDATION_ERROR",
  ...
  if (!LOGIN_ID_PATTERN.test(data.loginId)) throw new AppError({
    status: 400,
    code: "VALIDATION_ERROR",
    message: "아이디는 영문과 숫자 4~20자로 입력해주세요.",
  ...
  if (!data.termsAgreed) throw new AppError({
    status: 400,
    code: "VALIDATION_ERROR",
    message: "이용약관과 개인정보 처리방침에 동의해주세요.",
    details: { field: "termsAgreed" }
  });
...
export function validatePhoneVerify(body = {}) {
  ...
  if (!data.phoneVerificationId) throw new AppError({
    status: 400,
    code: "VALIDATION_ERROR",
    message: "휴대전화 인증 식별자가 필요합니다.",
    details: { field: "phoneVerificationId" }
  });
  if (!PHONE_CODE_PATTERN.test(data.code)) throw new AppError({
    status: 400,
    code: "VALIDATION_ERROR",
...
export function validatePhoneStatus(query = {}) {
  // 쿼리스트링에서 데이터 뽑아주기
  const data = {
    ...validatePhoneBase(query),
    phoneVerificationId: text(query.phoneVerificationId)
  };
  // verificationId 없으면 취소시켜주기
  if (!data.phoneVerificationId) throw new AppError({
    status: 400,
    code: "VALIDATION_ERROR",
    message: "휴대전화 인증 식별자가 필요합니다.",
...
export function validateFindLoginId(body = {}) {
  ...
  if (!data.name) throw new AppError({
    status: 400,
    code: "VALIDATION_ERROR",
    message: "이름을 입력해주세요.",
    details: { field: "name" }
...
export function validateLogin(body = {}) {
  ...
  if (!data.loginId || !data.password) throw new AppError({
    status: 400,
    code: "VALIDATION_ERROR",
...
export function validateLoginId(value) {
  ...
  if (!LOGIN_ID_PATTERN.test(loginId)) throw new AppError({
    status: 400,
    code: "VALIDATION_ERROR",
    message: "아이디는 영문과 숫자 4~20자로 입력해주세요.",
    details: { field: "loginId" }
