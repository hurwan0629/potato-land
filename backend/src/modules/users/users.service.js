...
function verifyEditToken(token, userIdx) {
  try {
    // 서명과 10분 만료 시간을 검증한 뒤 회원정보 수정 전용 토큰인지 확인한다.
    const payload = jwt.verify(token, env.jwt.accessToken.secret);

    // 다른 사용자가 발급받은 수정 토큰을 현재 계정에 사용할 수 없게 한다.
    if (payload.type !== "member_edit" || Number(payload.sub) !== Number(userIdx)) {
      throw new Error("invalid edit token");
    }
  } catch {
    throw new AppError(403, "EDIT_VERIFICATION_EXPIRED", "비밀번호 확인이 만료되었습니다. 다시 확인해주세요.");
  }
}
...
export async function verifyAccountPassword(userIdx, body) {
  // 1. 현재 비밀번호 입력 여부를 검증하고 로그인 사용자의 계정을 조회한다.
  const password = validatePasswordVerification(body);
  const user = await findEditableUser(userIdx);

  // 2. 탈퇴 계정 또는 일치하지 않는 비밀번호에는 수정 권한을 발급하지 않는다.
  ...
  // 3. 원래 비밀번호를 다시 전달하지 않도록 짧게 유효한 수정 토큰을 반환한다.
  return {
...
export async function updateMyAccount(userIdx, body) {
  // 1. 회원가입과 동일한 닉네임·전화번호·이메일·비밀번호 정책을 검증한다.
  const data = validateAccountUpdate(body);

  // 2. 비밀번호 확인 단계에서 발급한 수정 토큰의 사용자와 만료 시간을 확인한다.
  verifyEditToken(data.editToken, userIdx);

  // 3. 변경 전 정보와 비교하기 위해 현재 활성 계정을 조회한다.
  ...
  // 4. 전화번호가 달라진 경우 CHANGE_PHONE 목적으로 완료된 SMS 인증을 요구한다.
  ...
  // 5. 다른 회원이 이미 사용하는 닉네임·전화번호·이메일인지 확인한다.
  ...
  // 6. 새 비밀번호가 입력된 경우에만 해시를 만들어 기존 비밀번호를 교체한다.
  ...
  // 7. 사용한 전화번호 인증 상태를 소비하고 비밀번호 변경 시 모든 기기를 로그아웃한다.
...
export async function withdrawMyAccount(userIdx, body) {
  // 1. 추가 개인정보 입력 없이 앞 단계에서 발급한 수정 토큰으로 본인 여부를 확인한다.
  verifyEditToken(validateWithdrawal(body), userIdx);

  // 2. 사용자 데이터는 이력 보존을 위해 물리 삭제하지 않고 탈퇴 시각을 기록한다.
  await softDeleteUser(userIdx);

  // 3. 다른 기기를 포함한 모든 Refresh 세션을 제거해 추가 재발급을 막는다.
