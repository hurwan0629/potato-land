...
export const authApi = {
  ...
  sendPhoneCode: async (phone) => unwrap(await http.post("/auth/phone/send", { phone, purpose: "SIGNUP" })),
  /** 사용자가 입력한 인증번호가 서버의 발송 정보와 일치하는지 확인한다. */
  verifyPhoneCode: async ({ phone, phoneVerificationId, code }) => unwrap(await http.post("/auth/phone/verify", { phone, purpose: "SIGNUP", phoneVerificationId, code })),
